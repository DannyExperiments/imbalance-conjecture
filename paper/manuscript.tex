...
\hypersetup{
  pdftitle={The Imbalance Conjecture},
  pdfauthor={},
  pdfsubject={A proof of the Imbalance Conjecture, a threshold-set strengthening, and a one-equal-edge extension},
  pdfkeywords={graphical sequence, edge imbalance, Erdős--Gallai theorem, locally irregular graph}
}
...
We prove the Imbalance Conjecture of Kozerenko and Skochko: for every finite simple graph in which adjacent vertices have unequal degrees, the multiset of endpoint-degree differences is graphical.
More strongly, if $S\subseteq E(G)$ has $k$ edges and every edge of $S$ has imbalance at least $k$, then $S$ satisfies the corresponding subset Erd\H{o}s--Gallai inequality, whether or not $S$ consists of the largest imbalances.
We obtain quantitative slack and classify every equality case of this threshold theorem.
Finally, we prove that the full edge-imbalance multiset remains graphical when at most one edge of $G$ joins equal-degree vertices.
\end{abstract}
...
\tag{EG}
\end{equation}
The arbitrary-subset form of this criterion was already part of the original MathOverflow discussion and was recorded in Kozerenko's 2019 treatment \cite{KozerenkoMO2013,Kozerenko2019}.  Likewise, reductions to strong or reduced indices are prior degree-sequence machinery \cite{ZverovichZverovich1992,TripathiVijay2003}.  The new source-graph statement proved below is different: every $k$-edge set whose members all have source imbalance at least $k$ is forced to satisfy the corresponding known subset inequality.  Earlier threshold work on imbalanced edges controlled extremal counts rather than this Erd\H{o}s--Gallai deficit \cite{RautenbachSchiermeyer2006}.

\begin{theorem}[Imbalance Conjecture]\label{thm:main}
If $G$ is a finite simple locally irregular graph, then $\Ical(G)$ is graphical.
\end{theorem}

A documented search through August 1, 2026 found no retrievable earlier complete solution for finite simple graphs.  The main and strengthened results are therefore apparently new with moderate novelty confidence.  Absolute historical priority is not claimed.  Residual source gaps include an ambiguous inaccessible 2025 Serdiuk listing, an unidentified AI-summary/search snippet, one Scholar-indexed citation to the 2014 paper that could not be independently identified, incomplete subscription-database coverage, and possible unindexed or unpublished work.

\section{The arbitrary threshold-set theorem}
...
\begin{theorem}[Arbitrary threshold-set theorem]\label{thm:threshold}
...
\section{Deduction of the Imbalance Conjecture}\label{sec:deduction}
...
\section{Quantitative slack and equality}\label{sec:equality}

The preceding proof contains strict profile information.  Retain the notation $h=\lvert U\rvert$, $p=k-h$, and $e_u=s_u-1$ for a threshold set $S$ with $k\ge2$.
...
\section{One equal-degree edge}\label{sec:one-zero}

We now allow zero imbalances and prove a separate extension.  The argument requires explicit correction terms; it is not a formal consequence of Theorem~\ref{thm:main}.
...
DannyExperiments initiated and directed the investigation, selected the validation requirements, curated and validated the artifacts, and is the human curator and conventional manuscript author of the project.  OpenAI GPT-5.6 Pro generated the ordinary-language proof, strengthened results, and manuscript drafting.  OpenAI Codex coordinated the audit and provenance workflow and prepared the repository and reproducibility infrastructure.  No AI system is listed as an author.  The preserved mathematical and literature reports are AI-generated audits, not human peer review.  Human specialist review and proof-assistant verification have not been obtained.  The documented literature search supports only the qualified classification ``apparently new, moderate confidence.''
